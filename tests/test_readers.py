...
class TestBaseReader:
    def test_basic_functionality(self):
        ...
        for offset, n in [(0, 1), (4, 10), (10, 49)]:
            ...
            assert np.allclose(np.array(x),
                               np.arange(offset, offset + n)[:, None])
    ...
    @pytest.mark.parametrize("sigtype, dtype, shape, sigkw", [
        (pb.DualPolarizationSignal, np.complex64, (1024, 4, 2),
            {'center_freq': 1*u.GHz, 'pol_type': 'linear'}),
        (pb.FullStokesSignal, np.float64, (1024, 4, 4),
