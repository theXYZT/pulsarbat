...
class TestCoherentDedispersion:
    @pytest.mark.parametrize("dm", [10., 50., 100.])
    ...
    def test_basic(self, dm, use_dask):
        ...
        for ref_freq in [z.min_freq, z.center_freq, z.max_freq]:
            ...
            assert len(z) - len(y) >= DM.sample_delay(z.min_freq, z.max_freq,
                                                      z.sample_rate)

            d_st = y.start_time - z.start_time
            toffset = int(np.rint((d_st*z.sample_rate).to_value(u.one)))
            assert toffset >= DM.sample_delay(ref_freq, z.max_freq,
                                              z.sample_rate)
    ...
    def test_reversibility(self, seed):
        ...
        sos = scipy.signal.butter(10, 0.45, 'lowpass', fs=1.0,
                                  output='sos')
        x = scipy.signal.sosfilt(sos, x)

        sig = pb.BasebandSignal(x.reshape(-1, 1), sample_rate=SR,
                                center_freq=ref_freq, start_time=t0)
    ...
    def test_correctness(self, DM):
        ...
        sig1 = pb.BasebandSignal(x.reshape(-1, 1), sample_rate=SR,
                                 center_freq=ref_freq, start_time=Time.now())
...
class TestIncoherentDedispersion:
    @pytest.mark.parametrize("use_dask", [True, False])
    @pytest.mark.parametrize("dm", [50., 100., 200.])
    def test_basic(self, dm, use_dask):
        SR = 1 * u.kHz
        ref_freq = 1 * u.GHz
        shape = (8192, 32, 4)
        ...
        z1 = pb.FullStokesSignal(x, sample_rate=SR, start_time=t0,
                                 center_freq=ref_freq, chan_bw=8*u.MHz)
